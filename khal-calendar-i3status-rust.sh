...
display_date() {
    ...
    local icon
    text=$(date "$DATEFMT")
    short_text=$(date "$SHORTFMT")
    state="Idle"
    icon="calendar"

    echo "{\"text\": \"$text\", \"short_text\": \"$short_text\", \"state\": \"$state\", \"icon\": \"$icon\" }"
}
...
main() {
    ...
    declare -r SHORTFMT="${SHORTFMT:-+%Y-%m-%d}"
    ...
    case "$ACTION" in
        ...
        "show_today_events") show_events "Today's Events" "today" ;;
        "show_upcoming_events") show_events "Upcoming Events" "today" "tomorrow" ;;
        *) display_date ;;
    esac
